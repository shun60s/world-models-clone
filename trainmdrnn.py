...
parser.add_argument('--include_reward', action='store_true',
                    help="Add a reward modelisation term to the loss.")  # mse loss is defual 0 ? 
###
...
if args.include_reward:
    print ( "include_reward is True")
else:
    print ( "include_reward is False. ms loss will be 0.")

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# constants
BSIZE = args.batch_size #16
SEQ_LEN = 32
...
###
transform = transforms.Lambda(my_lambda)
    #my_lambda(x) )   # lambda x: np.transpose(x, (0, 3, 1, 2)) / 255)
...
def to_latent(obs, next_obs):
    ...
    with torch.no_grad():
        obs, next_obs = [
            f.interpolate(x.view(-1, 3, SIZE, SIZE), size=RED_SIZE,  # f.upsample(x.view(-1, 3, SIZE, SIZE), size=RED_SIZE,
...
def data_pass(epoch, train, include_reward): # pylint: disable=too-many-locals
    ...
    for i, data in enumerate(loader):
        ...
        """
        pbar.set_postfix_str("loss={loss:7.4f} bce={bce:7.4f} gmm={gmm:7.4f} mse={mse:7.4f}".format(
                                 loss=cum_loss / (i + 1), bce=cum_bce / (i + 1),
                                 gmm=cum_gmm / LSIZE / (i + 1), mse=cum_mse / (i + 1)))
