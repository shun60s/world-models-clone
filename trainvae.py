...
def loss_function(recon_x, x, mu, logsigma):
    """ VAE loss function """
    BCE = F.mse_loss(recon_x, x, reduction='sum')  # F.mse_loss(recon_x, x, size_average=False)
...
if __name__ == '__main__':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if not args.nosamples:
            with torch.no_grad():
                sample = torch.randn(RED_SIZE, LSIZE).to(device)  # 64, 32
                sample = model.decoder(sample).cpu()
                save_image(sample.view(64, 3, RED_SIZE, RED_SIZE), # 64, 3, 64, 64
